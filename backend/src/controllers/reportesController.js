...
const reportesController = {
  ...
  getInventario: async (req, res) => {
    ...
    if (!['Admin', 'Superadmin'].includes(req.user.rol)) {
      return res.status(403).json({ error: 'Acceso denegado. Se requiere rol Admin o Superadmin' });
    }
    ...
    try {
      const query = `
        ...
        SELECT
          ...
          codigo_id as id_producto,
          nombre_material as nombre,
          1 as cantidad,
          volumen_nominal as presentacion,
          marca,
          modelo as referencia,
          NULL as fecha_adquisicion,
          NULL as ubicacion,
          NULL as fecha_vencimiento
        FROM material_volumetrico
        ...
        SELECT
          ...
          codigo_identificacion as id_producto,
          nombre,
          1 as cantidad,
