...
const solicitudesController = {
  ...
  createCliente: async (req, res) => {
    ...
    try {
      ...
      if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ message: 'Cliente ya existe con ese número de identificación' });
      res.status(500).json({ message: 'Internal server error' });
    }
  },
  ...
  createSolicitud: async (req, res) => {
    ...
    try {
      // TiDB Serverless: solicitud_id no tiene AUTO_INCREMENT en tabla clustered
      if (!b.solicitud_id) {
        const [[maxRow]] = await pool.query('SELECT COALESCE(MAX(solicitud_id), 0) + 1 AS next_id FROM Solicitudes');
        b.solicitud_id = maxRow.next_id;
      }
